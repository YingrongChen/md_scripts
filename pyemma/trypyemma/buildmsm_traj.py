...
# ref_tica.save( prefix+'ref_tica.pyemma', overwrite=True, model_name='ref_tica')
ref_tica_traj = ref_tica.get_output() # get ref_tica coordinates
ref_tica_concatenated = np.concatenate(ref_tica_traj)
print('number of trajectories = ', np.shape(ref_tica_traj)[0]) #1
print('number of frames = ', np.shape(ref_tica_traj)[1]) #400000
print('number of TICA dimensions = ',np.shape(ref_tica_traj)[2]) #321
print('shape of ref_tica_concatenated = ',np.shape(ref_tica_concatenated)) #(400000, 321)

for i in range(0, np.shape(ref_tica_traj)[1], traj_size*5):
    data=[]
    for j in range(0,5):
        segment = ref_data[j:j+traj_size]
        data.append(segment)
    tica = ref_tica.transform(data)
    cluster = pyemma.coordinates.cluster_kmeans(tica, k=nclusters, max_iter=50)
    # cluster.save(prefix+'cluster.pyemma', overwrite=True, model_name='cluster')
    dtrajs = cluster.dtrajs
    dtrajs_concatenated = np.concatenate(cluster.dtrajs)

    fig, ax = plt.subplots(figsize=(4, 4))
    pyemma.plots.plot_density(
        *ref_tica_concatenated[:, :2].T, ax=ax, cbar=False, alpha=0.3)
    ax.scatter(*cluster.clustercenters[:, :2].T, s=5, c='C1')
    ax.set_xlabel('IC 1')
    ax.set_ylabel('IC 2')
    fig.tight_layout()
    plt.savefig(prefix+str(i)+"tica_cluster.png")
    plt.clf()
